...
class BackupController extends BaseController {
    ...
    public function actionIndex() {
		BaseController::$page_caption = 'Backup';
        $this->updateMenuItems();
        $path = $this->path;
        $dataArray = array();
    ...
    public function actionDownload($file)
    {
        ...
        if (file_exists($sqlFile)) {
            return Yii::$app->response->sendFile($sqlFile);
            // return $this->render('index', ['download' => Yii::$app->response->sendFile($sqlFile)]);
        }
    ...
    {
        BaseController::$page_caption = 'Restore';
        $model = new UploadForm();
        if (isset($_POST['UploadForm'])) {
            $model->attributes  = $_POST['UploadForm'];
            $model->upload_file = UploadedFile::getInstance($model, 'upload_file');
            $typeFile           = $model->upload_file->extension;
            if($typeFile == 'sql'){
                ini_set('max_execution_time', 0);
                $flashError = '';
                $flashMsg = '';
                ...
                Yii::$app->session->setFlash('danger', 'File harus extensi .sql');
            }
        }
        return $this->render('restoredatabase', array('model' => $model));
    }
}
